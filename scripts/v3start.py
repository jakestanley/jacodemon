...
def run(ui_manager: UIManager):
    Registry.get(LaunchService).Launch()
    # TODO ensure demos, badges and stats are refreshed
    ui_manager.set_state(UIState.SELECT_MAP)

def CreateViewsControllers(ui_manager: UIManager):
    view_config     = ViewConfig()
    view_map_select = ViewMapSelect()
    view_pre_launch = ViewPreLaunch()

    controller_config = ControllerConfig(view_config)
    ...
    controller_map_select = ControllerMapSelect(view_map_select)
    ...
    controller_pre_launch = ControllerPreLaunch(view_pre_launch)
    ui_manager.controllers.append(controller_pre_launch)
    controller_pre_launch.accept_signal.connect(lambda: run(ui_manager))
